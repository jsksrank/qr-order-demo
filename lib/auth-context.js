...
export function AuthProvider({ children }) {
  ...
  const resolveStore = useCallback(async (authUser) => {
    ...
    try {
      const { data, error: fetchError } = await supabase
        .from("stores")
        .select("id, store_name, plan, max_sku, bonus_sku, subscription_status, referral_code, referral_count")
  ...
  // サインアップ ★ S26: 第4引数 addressInfo / ★ S29: 第5引数 referrerCode を追加
  const signUp = async (email, password, shopName, addressInfo = {}, referrerCode = "") => {
    ...
    try {
      ...
      if (referrerCode && referrerCode.trim()) {
        ...
        // 見つからなくてもサインアップは続行（コード間違い程度でブロックしない）
      }

      // ★ 新規ストアの紹介コードを生成
      const newReferralCode = generateReferralCode();

      // stores テーブルに1行追加 ★ S26: postal_code, address, phone / ★ S29: referral_code, referred_by
